...
class MainRecylerAdapter(private val context: Context, val category: List<ViewType>) :
    ...
    inner class ItemListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView),HorizontelAdapter.OnHorzonatalClick
    {
        val Title: TextView
        val More: TextView
        ...
        fun bind(position: Int) {
         ...
         override fun onItemclickk(userlist: List<Item>, position: Int) {
            Navigation.findNavController(itemView).navigate(R.id.action_navigation_home_to_productDetatil)
        }
    }
    ...
    inner class ImageslidingHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int) {
            Toast.makeText(context, "$globalint", Toast.LENGTH_LONG).show()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == 1) {
            globalint+=1
            return ImageslidingHolder(LayoutInflater.from(context).inflate(R.layout.homecarousal, parent, false))
        }else{
            return ItemListViewHolder(LayoutInflater.from(context).inflate(R.layout.itemholder, parent, false))
        }
    }
    ...
    private fun setProductItemRecyler(recylerView: RecyclerView, category: List<Item>,itemView: View) {
    ...
    fun imageslider(imageslider: ImageSlider, ImaguRL: List<String>) {
        var slide_model: ArrayList<SlideModel> = ArrayList<SlideModel>()
